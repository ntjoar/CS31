...
{
    // TRIVIAL:  Return what column the zombie is at.
    return m_col;
}
...
void Player::moveOrAttack(int dir)
{
    ...
    int nZom = m_arena->numZombiesAt(r, c);
    if(nZom == 0)
...
void Arena::display() const
{
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++)
            if (numZombiesAt(r+1, c+1) == 1) //1 robot = value of the grid is R
                grid[r][c] = 'Z';
            else if (numZombiesAt(r+1,c+1) > 1 && numZombiesAt(r+1,c+1) < 9)
                grid[r][c] = '0' + numZombiesAt(r+1,c+1); //sets the value of the grid to the number of robots (the '0' + ... converts the int to the correct char)
...
bool Arena::addZombie(int r, int c)
{
    if(m_nZombies >= MAXZOMBIES)
        return false;
    if(r == m_player->row() && c == m_player->col()) //Don't spawn on player
        return false;
    else
    {
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    for(int k = 0; k < m_nZombies; k++)
    {
        if(m_zombies[k]->row() == r && m_zombies[k]->col() == c)
            if (m_zombies[k]->getAttacked(dir))
            {
                delete m_zombies[k];
...
// Note to Xcode users:  clearScreen() will just write a newline xinstead
